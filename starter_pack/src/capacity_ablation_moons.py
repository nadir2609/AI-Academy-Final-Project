"""
Capacity Ablation on Moons Dataset

This script trains 3 MLPs with different hidden widths {2, 8, 32}
to analyze how model capacity affects generalization on the moons dataset.
"""
...
def load_moons_data(data_path=None):
    """Load moons dataset (already split into train/val/test)."""
    if data_path is None:
        data_path = os.path.join(SCRIPT_DIR, '..', 'data', 'moons.npz')
...
def run_capacity_ablation():
    ...
    # Load data
    X_train, y_train, X_val, y_val, X_test, y_test = load_moons_data()
    
    # Configuration
    hidden_widths = [2, 8, 32]
    input_dim = 2  # Moons has 2 features
    ...
    print("=" * 70)
    print("Capacity Ablation on Moons Dataset")
    print("=" * 70)
    print(f"{'Hidden Width':<15}{'Train Acc':<12}{'Val Acc':<12}{'Test Acc':<12}{'Boundary'}")
    print("-" * 70)
    
    boundary_labels = {2: 'Simple curve', 8: 'Moderate curve', 32: 'Complex curve'}
    ...
    for hidden_dim in hidden_widths:
        ...
        print(f"{hidden_dim:<15}{train_acc*100:>10.1f}%{val_acc*100:>10.1f}%{test_acc*100:>10.1f}%    {boundary_labels[hidden_dim]}")
