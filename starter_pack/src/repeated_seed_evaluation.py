...
from helper import load_dataset, softmax
...
def run_repeated_seed_evaluation(seeds=[0, 1, 2, 3, 4]):
    ...
    for model_name in ['softmax', 'mlp']:
        ...
        print(f"                     (95% CI: [{acc_stats['ci_lower']*100:.2f}%, {acc_stats['ci_upper']*100:.2f}%])")
...
+------------------+------------------------+------------------------+""")
    ...
    for model_name in ['softmax', 'mlp']:
        ...
        print(f"| {name:<16} | {acc['mean']*100:.2f}% ± {acc['margin']*100:.2f}%         | {ce['mean']:.4f} ± {ce['margin']:.4f}         |")
